...
Builds a Node-20-on-Debian replacement for `cr.kagent.dev/kagent-dev/kagent/ui:0.10.2` so the Next.js process doesn't `SIGILL` on the HP server's older x86-64 CPU.

**This is a permanent fork.** Upstream closed [#1505](https://github.com/kagent-dev/kagent/issues/1505) as `not_planned` on 2026-05-23 and still ships Node 24 as of v0.10.2, so this image will not go away. **Every kagent chart bump requires a matching rebuild here** — see "Bumping `KAGENT_VERSION`" below.
...
3. `docker buildx build --platform linux/amd64 --push` to `852893458518.dkr.ecr.us-east-2.amazonaws.com/kagent-ui:0.10.2-node20`
