...
- **Break-glass, any of the above:** the local `admin` account is still enabled and does not involve Dex or GitHub. Password: `kubectl -n argo-cd get secret argocd-initial-admin-secret -o jsonpath='{.data.password}' | base64 -d` (absent if it was rotated or deleted after install).
