...
- **GitOps safety — Argo CD must ignore `catalog-info.yaml`.** These files are Backstage entities (`apiVersion: backstage.io/v1alpha1`), not Kubernetes manifests, but they live inside Argo CD-synced app directories (`base-apps/<app>/`). Without action, Argo CD would try to apply them and fail sync (no `backstage.io` CRD exists). **Mechanism (post-implementation correction):** each app's Argo CD `Application` sets `spec.source.directory.exclude: catalog-info.yaml` — an in-band guard Argo CD honors at render time. The validator enforces this per app. A global `resource.exclusions` in `argocd.tf` was originally planned (and briefly added) but proved **ineffective** — the argocd module writes the deprecated Helm `server.config.*` path while the chart reads `configs.cm.*`, so it never reached the live `argocd-cm`; it was removed, and the per-app guard is the sole mechanism. See the note in `argocd.tf` and `templates/agent-docs/README.md`.
...
- **Per-app `directory.exclude`** (the sole mechanism). The global `resource.exclusions` approach was found ineffective (module writes deprecated `server.config.*`; chart reads `configs.cm.*`) and removed. The validator's `check_app_directory_exclude` enforces that each in-scope app's Argo CD `Application` (matched by `spec.source.path`) excludes `catalog-info.yaml`.
