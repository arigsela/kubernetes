...
T18|.|SUC hop workers → `v1.34.9+k3s1`. gate §V.1,5,6,11,14,28 — post-relocate THIS hop = Vault + CNPG outage (§V.37), ⊥ bare "verify §V.5". NB post-§T.37 the CNPG PDB blocks draining `k3s-worker-01` (§V.42) — scale→2 \| disable PDB for the hop + §V.49 istio-cni/ztunnel check. **! §V.50: RESTART `istio-cni-node` on EACH worker the moment SUC's k3s restart completes there.** SUC ⊥ do this ∴ an UNATTENDED hop breaks pod-create on that worker exactly as §B.7 broke control-01, & §V.49's DS-Ready check ⊥ detect it (all 3 stayed Ready). do §T.46 first if possible|V4,V5,V6,V17,V31,V37,V42,V50
