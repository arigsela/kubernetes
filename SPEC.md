...
T37|.|relocate `postgresql-cluster` → `k3s-worker-01`, 3-phase (§V.43): suspend `master-app`+app (§V.33) → (a) BROADEN selector, control-01 still matches → verify restart = no-op → (b) cordon `k3s-control-01` (+`k3s-worker-02`: 8.8GB free, ⊥ target) → `instances` 1→2 → wait streaming → auto-switchover → ASSERT `-2` = primary (§V.38) → scale→1 drops original → uncordon → (c) NARROW → `kubernetes.io/hostname: k3s-worker-01`. ∀ live patch `--type json` (§V.44). resume Argo|V6,V29,V33,V38,V42,V43,V44
